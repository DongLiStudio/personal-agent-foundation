...
这套基座把原本分散在不同平台中的授权信息汇入同一个 Agent 工作上下文：
...
- **项目状态统一理解**：结合 GLOBAL 项目索引和各项目持续上下文，让 Agent 能同时理解个人任务、日程、知识和项目进度。
...
5. 默认助手项目名称。
6. Obsidian Vault 路径，或“未配置”；如已配置，安装验证后还会与你确认实际目录结构、优先入口、允许读取或写入的范围和明确排除项，再生成专属的 `OBSIDIAN_LINK.md`。
