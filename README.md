...
7. GLOBAL 和 Skills 恢复完成后，再分别询问是否现在连接飞书、GitHub 和 Obsidian；默认推荐并预选“现在连接”。用户选择连接飞书时，默认创建新的飞书应用和新的专用 Profile，本机已有 Profile 或其他应用只能在用户明确选择高级迁移/复用路径后使用，不能自动复用。用户选择连接 GitHub 或 Obsidian 时，由对应 Skill 或官方 CLI 打开授权、验证码或网页登录流程，并回读真实身份或路径后写回 GLOBAL。只有用户主动选择稍后再配或当前没有账号/Vault 时，才保留未配置说明。
...
用户选择连接 Obsidian 时，再填写 Vault 路径并确认实际目录结构、优先入口、允许读取或写入的范围和明确排除项，生成专属的 `OBSIDIAN_LINK.md`。
