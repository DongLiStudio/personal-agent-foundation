...
本项目不会提供独立 GUI。宿主 Agent 支持图形提问、选项卡片、目录选择或可视化确认时，应优先使用宿主界面；例如 Codex 可在支持的环境中通过可视化能力展示安装选项和确认摘要。没有图形能力时使用普通对话。
...
4. 主动询问是否现在连接飞书、GitHub 和 Obsidian。不能把这些能力静默设为“未配置”；只有用户明确选择稍后再配、当前没有账号或当前没有 Vault 时，才写入未配置说明。
5. 自动检测宿主或系统中的 Python 3.11+；缺失时先展示官方安装方案，经确认后自动安装。
6. 收集非敏感配置并执行模板审计、安装计划和 dry-run。
7. 只有在展示计划并获得明确确认后，才执行真实安装和验证。
...
2. 是否现在连接飞书账号；如果连接，再填写默认飞书 Profile。
3. 是否现在连接 GitHub 账号；如果连接，再填写默认 GitHub 账号。
4. IANA 时区，例如 `Asia/Shanghai`。
5. 默认助手项目名称。
6. 是否现在连接 Obsidian Vault；如果连接，再填写 Vault 路径。安装验证后还会与你确认实际目录结构、优先入口、允许读取或写入的范围和明确排除项，再生成专属的 `OBSIDIAN_LINK.md`。
...
→ 用户确认 → 安装 → 独立验证 → 可选知识库结构确认与连接
→ 初始化 GLOBAL 本地 Git → 恢复 Skills → 引导账号连接
