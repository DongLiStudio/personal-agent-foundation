...
- 宿主提供图形化提问、选择目录、选项卡片或确认能力时优先使用；界面由宿主 Agent 生成，本产品不实现独立 GUI。
- 没有图形能力时使用普通对话逐项收集。
- 已有明确答案不重复询问；路径、连接意图和外部身份在执行前回读确认。
- 不得把飞书、GitHub 或 Obsidian 静默设为“未配置”。必须先分别询问用户是否现在连接；用户明确选择暂不连接、当前没有账号或当前没有 Vault 后，才可写入未配置说明。
...
2. 收集六项非敏感配置：
   ...
   - 飞书连接意图：询问“是否现在连接飞书账号或创建默认飞书 Profile”。用户选择连接时再收集默认 Profile 名；选择稍后再配时写入明确的未配置说明。
   - GitHub 连接意图：询问“是否现在连接 GitHub 账号并设置默认账号”。用户选择连接时再收集默认账号；选择稍后再配时写入明确的未配置说明。
   - IANA 时区。
   - 通用助手项目名。
   - Obsidian 连接意图：询问“是否现在连接 Obsidian Vault”。用户选择连接时再收集 Vault 路径；选择稍后再配或没有 Vault 时写入明确的未配置说明。用户提供真实 Vault 时只记录路径，结构映射留到安装验证后对话确认。
...
10. 用户配置了真实 Vault 时，完整读取 `references/obsidian-layout.md`，通过对话和经授权的浅层只读检查理解用户实际目录结构、入口文件和读写边界；展示拟写入的映射并取得确认后，只更新已安装实例中的 `GLOBAL/OBSIDIAN_LINK.md`，再创建并验证 `GLOBAL/obsidian-resource` Junction 或 symlink。不得修改产品模板、强套作者结构或递归扫描 Vault。未配置时保持通用说明，不创建伪链接。
11. 在新 `GLOBAL` 中初始化本地 Git；完成敏感信息扫描后创建包含已确认 Obsidian 配置的安装基线提交。不创建 remote，不 push。
12. 从 `GLOBAL/.agents/skills/` 恢复当前宿主可发现的全局 Skill，并按 `SKILL_DEPENDENCIES.md` 恢复主动安装的外部 Skills。现有同名安装先比较文件，不能静默覆盖。
13. 使用已恢复的 `feishu-profile` 和 `github-cli` 分别完成登录、默认身份核验和回读；网页登录由用户亲自确认，不处理明文凭据。
...
- GLOBAL 本地 Git、Skill 恢复、默认飞书/GitHub 路由和可选知识库连接均有独立回读结果；`OBSIDIAN_LINK.md` 已匹配用户确认的 Vault 结构，或明确记录为未配置。
