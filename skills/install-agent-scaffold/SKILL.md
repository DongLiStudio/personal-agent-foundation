...
2. 预检 Python 3.11+，并用 `zoneinfo.ZoneInfo(<IANA 时区>)` 验证时区数据；缺失时把 `python -m pip install tzdata` 列入安装计划并在确认后执行，再回读验证。
3. 将配置写入当前工作目录或系统临时目录中的临时 JSON；不要写进产品仓库、GLOBAL 模板或长期文档。
4. 运行 `audit-template`，确认模板清单、占位符、编码、换行和链接边界。
5. 运行 `plan`，展示目标、文件数和风险；取得用户安装确认。
6. 运行 `install`。不得对已存在目标使用覆盖、合并或删除绕过保护。
7. 运行独立 `verify`，确认必需文件、编码和占位符残留。
8. 在新 `GLOBAL` 中初始化本地 Git；完成敏感信息扫描后创建安装基线提交。不创建 remote，不 push。
9. 从 `GLOBAL/.agents/skills/` 恢复当前宿主可发现的全局 Skill，并按 `SKILL_DEPENDENCIES.md` 恢复主动安装的外部 Skills。现有同名安装先比较文件，不能静默覆盖。
10. 使用已恢复的 `feishu-profile` 和 `github-cli` 分别完成登录、默认身份核验和回读；网页登录由用户亲自确认，不处理明文凭据。
11. 用户配置了真实 Vault 时，根据系统创建 `GLOBAL/obsidian-resource` Junction 或 symlink，并验证链接目标；未配置时保持缺失，不创建伪链接。
12. 执行 `references/onboarding.md` 的全局提示词和首次项目教程。
13. 删除临时配置和 staging，输出完成项、人工步骤、未验证项及建议下一步。
