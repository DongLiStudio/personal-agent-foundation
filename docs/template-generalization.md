...
模板以真实 GLOBAL 的 57 个治理基线文件为结构来源，并增加公开产品自身的恢复状态与 `restore-agent-foundation` 模块；个人服务器地址、账号、指纹和服务拓扑不进入公开模板。
...
- 服务器文件只保留 Profile 路由结构、主机指纹核验、身份文件安全恢复和授权边界，不包含真实地址、账号、指纹、私钥路径或服务拓扑。
...
当前模板受管文件数为 64：57 个治理基线文件、1 个 `FOUNDATION_STATE.json` 和 6 个恢复 Skill 文件。有意新增或删除 GLOBAL 模块时，必须同时更新 `template-manifest.json`、本文件和测试。不能为了通过脱敏扫描静默漏掉源文件。
