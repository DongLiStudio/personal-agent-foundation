...
- `ALIYUN_PROFILES.md`：阿里云 CLI 多 Profile 与云效组织授权上下文的非敏感映射；不记录 AK/SK、PAT 或票据。
- `SERVER_PROFILES.md`：跨项目服务器 Profile 索引、选择规则和共通操作边界；不记录每台服务器的完整细节。
- `servers/`：每台受管服务器一个详情文件，记录该 Profile 的非敏感连接事实、服务归属、安全边界和恢复信息；公开模板默认只提供登记说明。
...
- `ALIYUN_PROFILES.md`：涉及阿里云 CLI、云效组织、Codeup、Flow、云资源账号或授权路由时读取。
- `SERVER_PROFILES.md`：涉及 SSH、服务器、共享基础设施、部署目标或服务归属时先读取，用于解析唯一服务器 Profile。
- `servers/<profile>.md`：解析出服务器 Profile 后只读取对应详情；不要默认递归或批量读取全部 `servers/`。
