...
- 默认只读审计和计划。实施改动、提交和其他外部系统操作前必须有明确授权；已有 remote/upstream 的后续 push 遵循 GLOBAL 中已记录的持续推送授权，不再逐次重复询问。
- 保护各项目现有未提交内容。每个项目分别检查、验证和提交；不跨仓库暂存，不混入无关改动。完成并验证聚焦提交后，如目标 remote/upstream 已配置且用户已授予持续推送，立即 push 并回读；首次创建远端、首次 push、MR/合并、Release 和部署仍需遵循各自授权边界。
...
- 每个仓库单独提交，提交信息聚焦本项目 GLOBAL 对齐；已有且已核验 remote/upstream、且用户已记录持续授权时，提交后立即 push 并回读，无法推送则把原因列为未闭环项。
